...
module.exports = {
    ...
    exe(message, args, handler, game, player) {
       ...
       if (game.clue) return "**✖ | You already gave a clue to your team!**";
       let [word, number] = args;
       if (isNaN(number)) return "**✖ | You need to provide the amount of guesses your team will have!**";
       number = Number(number);
       game.lastAction = Date.now();
       if (number > game.words.unguessed().size) return `**✖ | Maximum guesses are ${game.words.unguessed().size}**`;
       game.clue = `${word} (${number})`;
       handler.sendToChannel(message.channel_id, `${player.team.emoji} | Clue for the **${player.team}** team: **${word}** (${number})`);
