...
function handleWinner(game, res, handler) {
    const winner = game.isThereAWinner();
    if (typeof winner === "string") {
        game.masterBoard.sendAsMessage(handler, game.channel, `${res}\n${winner}`);
        handler.games.delete(game.channel);
        return true;
    }
    if (winner) {
...
module.exports = {
    ...
    exe(message, args, handler, game, player) {
        ...
        if (player.team.guesses === false) return "**✖ | The spymaster hasn't given the clue!**";
        if (player.team.spymaster.user.id == message.author.id) return "**✖ | The spymaster cannot use this command!**";
        if (!args.length) return "**✖ | You need to provide at least 1 word!**";
       if (args[0].includes(" | ")) args = args[0].split(" | ");
        if (!args.length) return "**✖ | You need to provide at least 1 word!**";
        if (args.length > player.team.guesses) return "**✖ | You cannot guess this many words!**";
        let res = '';
        let sent = false;
        for (let wordA of args) {
            let og = game.words.find(w => w.word == wordA);
            if (player.team.guesses === 0) {
                if (handleWinner(game, res, handler)) return;
                game.turn.canEnd = false;
                game.turn.guesses = false;
                game.clue = null;
                game.turn = game.other(player.team);
                handler.sendToChannel(message.channel_id, `${res}\n**${game.turn.emoji} | \`${game.turn}\` (${game.turn.players.map(p => p.user.username).join(", ")}), it's your turn!**`);
                game.displayBoard();
