...
export default {
    ...
    execute: async ({game, player, interaction, args}: CommandContext) : Promise<CommandExecuteRes> => {
        ...
        if (args.player) {
            if (!guildDb.gameMaster) return "> ❌ | Only people with the game master role can force who becomes the spymaster!";
            else if (!interaction.member.roles.includes(guildDb.gameMaster)) return `> ❌ | You need the <@&${guildDb.gameMaster}> role in order to force who becomes the spymaster!`;
